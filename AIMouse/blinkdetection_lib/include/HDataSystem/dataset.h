...
public:
    ...
    void init(int channels, int epochlen)
    {
        ...
        this->m_epochlen = epochlen;

        this->m_event = 0;
        if (this->m_channels * this->m_epochlen > 0)
            this->m_data = new double[channels * epochlen];
        else
            this->m_data = 0;
    ...
    {
        if (m_pointindex < m_epochlen) {
            for (int ch = 0; ch < m_channels; ch++) {
                this->m_data[m_pointindex * m_channels + ch] = data[ch];
            }
            m_pointindex += 1;
    ...
    void insertSample(double * data, const std::vector<int> &chs)
    {
        if (m_pointindex < m_epochlen) {
            for (int ch = 0; ch < m_channels; ch++) {
                this->m_data[m_pointindex * m_channels + ch] = data[chs[ch]];     //注意：有一个隐含的前提：chs[ch]的值一定是在data长度范围内，否则报错，待优化
            }
            m_pointindex += 1;
    ...
    bool isFull()
    {
        return (m_pointindex >= m_epochlen);
    }
};
