...
void ChooseDevice::on_m_pbConnect_clicked()
{
    ...
    for(int r = 0; r < row; r++){
        for(int c = 0; c < col; c++){
            ...
            if(it->isSelected()){
                deviceName = m_deviceList->item(r, 1)->text();
                qDebug() << deviceName;
            }
        }
    ...
    //发送连接设备名，使两个实例连接
    emit checkSignal(hnnk::DataAppOperator::DAO_CONNET, deviceName);
    qDebug()<<" 获取到的连接的设备名： "<<deviceName;
    this->hide();
